...
# 1. Fuse the training and the test sets to create one data set.
...
# Set working directory to the location where the UCI HAR Dataset was unzipped
	setwd('/coursera_data/UCI HAR Dataset/');
...
# Create a vector for the column names from the finalData, used to select the desired mean() & stddev() columns
	colNames  = colnames(finalData); 

# 2. Extract only the measurements on the mean and standard deviation for each measurement. 
# Create a logicalVector that contains TRUE values for the ID, mean() & stddev() columns and FALSE for  
others
	logicalVector = (grepl("activity..",colNames) | grepl("subject..",colNames) | grepl("-mean..",colNames) &  
	!grepl("-meanFreq..",colNames) & !grepl("mean..-",colNames) | grepl("-std..",colNames) & !grepl("-std 
	()..-",colNames));
...
# 5. Create tidy data set with the average of each variable (activity and subject). 
# Create a new table, finalDataNoActivityType without the activityType column
	finalDataNoActivityType  = finalData[,names(finalData) != 'activityType'];

# Summarizing the finalDataNoActivityType table to include mean of each variable (activity and subject)
